            ...
            for (int j = 0; j < sampleCount; j++) {
                ...
                if(pathLength >= 1 && currentSampleBSDFPath.vertices.at(pathLength - 1).vertexType == EMITTER) {
                    if(pathLength == 1) { //Direct hit emitter
                        ...
                        Ray finalBounceRay{};
                        finalBounceRay.o = currentSampleBSDFPath.vertices.at(pathLength - 2).hitPointAndMaterial.hitInfo.intersectionPoint;
                        finalBounceRay.d = glm:: normalize(currentSampleBSDFPath.vertices.at(pathLength - 1).hitPointAndMaterial.hitInfo.intersectionPoint
                                                           - currentSampleBSDFPath.vertices.at(pathLength - 2).hitPointAndMaterial.hitInfo.intersectionPoint);

                        //Find Le in the given direction of final shot ray
                        L = currentSampleBSDFPath.vertices.at(pathLength - 1).hitPointAndMaterial.closestObject->Le(finalBounceRay);
