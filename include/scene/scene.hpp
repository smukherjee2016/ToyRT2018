...
public:
    ...
    void makeScene() {
        ...
        objects.emplace_back(std::make_unique<Sphere>(Point3(1e5+99,40.8,81.6)  , 1e5 , std::make_shared<LambertUniform>(Spectrum(.25,.25,.75)))); // Right wall (after modification
        objects.emplace_back(std::make_unique<Sphere>(Point3(-1e5+1,40.8,81.6), 1e5 , std::make_shared<LambertUniform>(Spectrum(.75,.25,.25)))); //Left wall (after modification)
        objects.emplace_back(std::make_unique<Sphere>(Point3(50,40.8,-1e5)      , 1e5 , std::make_shared<LambertUniform>(Spectrum(.75)))); //Front wall (after modification)
        objects.emplace_back(std::make_unique<Sphere>(Point3(50,1e5+81.6,81.6)      , 1e5 , std::make_shared<LambertUniform>(Spectrum(.75)))); //Ceiling (after modification)
